...
void COPCItem::getSupportedProperties(CAtlArray<CPropertyDescription> &desc){
	...
	USES_CONVERSION;
    HRESULT res = group.getServer().getPropertiesInterface()->QueryAvailableProperties(name.GetBuffer(), &noProperties, &pPropertyIDs, &pDescriptions, &pvtDataTypes);
	if (FAILED(res)){
...
void COPCItem::getProperties(const CAtlArray<CPropertyDescription> &propsToRead, ATL::CAutoPtrArray<SPropertyValue> &propsRead){
	...
	USES_CONVERSION;
    HRESULT res = group.getServer().getPropertiesInterface()->GetItemProperties(name.GetBuffer(), noProperties, pPropertyIDs, &pValues, &pErrors);
	delete []pPropertyIDs;
	if (FAILED(res)){
